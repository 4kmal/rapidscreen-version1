"use client";

import { useTheme } from "next-themes";
import { useEffect, useState } from "react";
import { motion } from "framer-motion";

type ThemeOption = "light" | "dark" | "system";

export function ThemeToggle() {
  const { theme, setTheme, resolvedTheme } = useTheme();
  const [mounted, setMounted] = useState(false);

  useEffect(() => {
    setMounted(true);
  }, []);

  if (!mounted) {
    return (
      <div className="h-28 w-[84px] rounded-full bg-black-alpha-4" />
    );
  }

  const currentTheme = (theme === "system" ? "system" : theme) as ThemeOption;
  const isDarkResolved = resolvedTheme === "dark";

  const getButtonPosition = () => {
    switch (currentTheme) {
      case "light": return 2;
      case "dark": return 28;
      case "system": return 54;
      default: return 2;
    }
  };

  return (
    <div
      className="relative h-28 w-[84px] rounded-full bg-black-alpha-4 flex items-center"
      role="radiogroup"
      aria-label="Theme"
      style={{
        boxShadow: "inset 0px 0px 2px rgba(0, 0, 0, 0.06), inset 0px 0px 1px rgba(0, 0, 0, 0.04)"
      }}
    >
      {/* Sliding button indicator */}
      <motion.div
        className="absolute w-24 h-24 rounded-full bg-background-base"
        style={{
          boxShadow: "0px 1px 3px rgba(0, 0, 0, 0.1), 0px 1px 2px rgba(0, 0, 0, 0.06)"
        }}
        animate={{ left: getButtonPosition() }}
        transition={{ duration: 0.15, ease: [0.4, 0, 0.2, 1] }}
      />

      {/* Light mode */}
      <label
        className="relative flex-1 h-full flex items-center justify-center cursor-pointer z-10 group"
        title="Light"
      >
        <input
          type="radio"
          name="theme"
          value="light"
          checked={currentTheme === "light"}
          onChange={() => setTheme("light")}
          className="sr-only"
          aria-label="Light theme"
        />
        <svg
          width="12"
          height="13"
          viewBox="0 0 12 13"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
          className={`transition-colors duration-150 ${
            currentTheme === "light" ? "text-[#facc15]" : "text-black-alpha-24"
          }`}
        >
          <path
            d="M6 4C5.33696 4 4.70107 4.26339 4.23223 4.73223C3.76339 5.20107 3.5 5.83696 3.5 6.5C3.5 7.16304 3.76339 7.79893 4.23223 8.26777C4.70107 8.73661 5.33696 9 6 9C6.66304 9 7.29893 8.73661 7.76777 8.26777C8.23661 7.79893 8.5 7.16304 8.5 6.5C8.5 5.83696 8.23661 5.20107 7.76777 4.73223C7.29893 4.26339 6.66304 4 6 4V4Z"
            fill="currentColor"
          />
          <path
            d="M9.89998 6.49997H10.7978M8.69998 9.19997L8.99998 9.49997M5.99998 10.4L6.00106 11.2989M1.19997 6.49997H2.09781M2.99997 3.49997L3.30004 3.80003M5.99998 1.69996L6.00106 2.59888M3.29997 9.19997L2.99997 9.49997M8.69998 3.79997L8.99998 3.49997"
            stroke="currentColor"
            strokeLinecap="round"
            className={`transition-all duration-200 ${
              currentTheme !== "light" ? "opacity-60" : ""
            }`}
          />
        </svg>
      </label>

      {/* Dark mode */}
      <label
        className="relative flex-1 h-full flex items-center justify-center cursor-pointer z-10 group"
        title="Dark"
      >
        <input
          type="radio"
          name="theme"
          value="dark"
          checked={currentTheme === "dark"}
          onChange={() => setTheme("dark")}
          className="sr-only"
          aria-label="Dark theme"
        />
        <svg
          width="10"
          height="11"
          viewBox="0 0 10 11"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
          className={`transition-colors duration-150 ${
            currentTheme === "dark" ? "text-accent-black" : "text-black-alpha-24"
          }`}
        >
          <path
            d="M3.75077 0.673362C3.81179 0.74751 3.84918 0.838253 3.85809 0.933867C3.86701 1.02948 3.84704 1.12557 3.80078 1.20972C3.43937 1.87328 3.25063 2.61707 3.25192 3.37267C3.25192 5.88631 5.30109 7.92173 7.82661 7.92173C8.15606 7.92173 8.47675 7.88735 8.78493 7.82171C8.87941 7.80125 8.97781 7.80908 9.06787 7.84421C9.15792 7.87934 9.23563 7.94022 9.29129 8.01925C9.35005 8.10142 9.38 8.20071 9.3765 8.30166C9.37299 8.40262 9.33622 8.49959 9.27191 8.57749C8.78161 9.17976 8.16309 9.66502 7.46143 9.9979C6.75978 10.3308 5.99268 10.5029 5.21607 10.5016C2.33423 10.5016 0 8.17991 0 5.31932C0 3.16638 1.32152 1.31975 3.20316 0.537084C3.29689 0.497471 3.40094 0.48934 3.49969 0.513914C3.59844 0.538488 3.68654 0.594439 3.75077 0.673362Z"
            fill="currentColor"
          />
          <path
            d="M8.83559 3.10006C8.82942 3.11904 8.81786 3.13551 8.80252 3.14716C8.78717 3.15881 8.76881 3.16507 8.74998 3.16507C8.73114 3.16507 8.71278 3.15881 8.69744 3.14716C8.68209 3.13551 8.67053 3.11904 8.66436 3.10006L8.50313 2.58398C8.43127 2.35328 8.26192 2.17259 8.0457 2.09591L7.56201 1.92389C7.54422 1.91731 7.52879 1.90497 7.51787 1.8886C7.50695 1.87223 7.50108 1.85264 7.50108 1.83254C7.50108 1.81245 7.50695 1.79285 7.51787 1.77648C7.52879 1.76011 7.54422 1.74778 7.56201 1.7412L8.0457 1.56917C8.15225 1.53135 8.24908 1.46755 8.32848 1.38282C8.40789 1.2981 8.46769 1.19479 8.50313 1.0811L8.66436 0.565025C8.67053 0.54604 8.68209 0.529578 8.69744 0.517927C8.71278 0.506275 8.73114 0.500014 8.74998 0.500014C8.76881 0.500014 8.78717 0.506275 8.80252 0.517927C8.81786 0.529578 8.82942 0.54604 8.83559 0.565025L8.99682 1.0811C9.03227 1.19479 9.09206 1.2981 9.17147 1.38282C9.25088 1.46754 9.3477 1.53135 9.45426 1.56917L9.93794 1.7412C9.95573 1.74778 9.97116 1.76011 9.98208 1.77648C9.993 1.79285 9.99887 1.81245 9.99887 1.83254C9.99887 1.85264 9.993 1.87223 9.98208 1.8886C9.97116 1.90497 9.95573 1.91731 9.93794 1.92389L9.45426 2.09591C9.23803 2.17259 9.06868 2.35328 8.99682 2.58398L8.83559 3.10006Z"
            fill="currentColor"
            className={`transition-all duration-200 origin-bottom ${
              currentTheme !== "dark" ? "opacity-0 -rotate-45" : ""
            }`}
          />
          <path
            d="M7.00397 6.40119C6.99501 6.43007 6.97775 6.45519 6.95464 6.473C6.93153 6.49081 6.90374 6.5004 6.87524 6.5004C6.84673 6.5004 6.81895 6.49081 6.79584 6.473C6.77273 6.45519 6.75546 6.43007 6.7465 6.40119L6.50466 5.62641C6.39655 5.28103 6.14284 5.01032 5.81913 4.89497L5.09298 4.63694C5.06592 4.62737 5.04237 4.60896 5.02567 4.5843C5.00898 4.55964 5 4.53 5 4.49958C5 4.46917 5.00898 4.43952 5.02567 4.41487C5.04237 4.39021 5.06592 4.37179 5.09298 4.36223L5.81913 4.10419C5.97878 4.04744 6.12385 3.95179 6.24284 3.82484C6.36183 3.69788 6.45147 3.54309 6.50466 3.37275L6.7465 2.59797C6.75546 2.5691 6.77273 2.54397 6.79584 2.52616C6.81895 2.50835 6.84673 2.49877 6.87524 2.49877C6.90374 2.49877 6.93153 2.50835 6.95464 2.52616C6.97775 2.54397 6.99501 2.5691 7.00397 2.59797L7.24581 3.37275C7.299 3.54309 7.38864 3.69788 7.50763 3.82484C7.62662 3.95179 7.77169 4.04744 7.93134 4.10419L8.65749 4.36223C8.68456 4.37179 8.70811 4.39021 8.7248 4.41487C8.74149 4.43952 8.75047 4.46917 8.75047 4.49958C8.75047 4.53 8.74149 4.55964 8.7248 4.5843C8.70811 4.60896 8.68456 4.62737 8.65749 4.63694L7.93134 4.89497C7.77169 4.95173 7.62662 5.04737 7.50763 5.17433C7.38864 5.30129 7.299 5.45607 7.24581 5.62641L7.00397 6.40119Z"
            fill="currentColor"
            className={`transition-all duration-200 origin-bottom ${
              currentTheme !== "dark" ? "opacity-0 -rotate-45" : ""
            }`}
          />
        </svg>
      </label>

      {/* Auto/System mode */}
      <label
        className="relative flex-1 h-full flex items-center justify-center cursor-pointer z-10 group"
        title="Auto"
      >
        <input
          type="radio"
          name="theme"
          value="system"
          checked={currentTheme === "system"}
          onChange={() => setTheme("system")}
          className="sr-only"
          aria-label="System theme"
        />
        <svg
          width="10"
          height="9"
          viewBox="0 0 10 9"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
          className={`transition-colors duration-150 ${
            currentTheme === "system"
              ? isDarkResolved ? "text-accent-black" : "text-[#facc15]"
              : "text-black-alpha-24"
          }`}
        >
          <path
            d="M9 0H1C0.45 0 0 0.45 0 1V6.5C0 7.05 0.45 7.5 1 7.5H2.5C2.225 7.775 2 7.935 2 8.295C2 8.685 2.315 9 2.705 9H7.29C7.68 9 7.995 8.685 7.995 8.295C7.995 7.935 7.775 7.78 7.495 7.5H8.995C9.545 7.5 9.995 7.05 9.995 6.5V1C10 0.45 9.55 0 9 0ZM9 6.5H1V1H9V6.5Z"
            fill="currentColor"
          />
          <path
            d="M6.53428 2.97561C6.53181 2.98273 6.52718 2.98891 6.52103 2.99328C6.51489 2.99765 6.50754 3 6.5 3C6.49246 3 6.48511 2.99765 6.47897 2.99328C6.47282 2.98891 6.46819 2.98273 6.46572 2.97561L6.40118 2.78196C6.3724 2.6954 6.3046 2.6276 6.21804 2.59882L6.02439 2.53428C6.01727 2.53181 6.01109 2.52718 6.00672 2.52104C6.00235 2.51489 6 2.50754 6 2.5C6 2.49246 6.00235 2.48511 6.00672 2.47897C6.01109 2.47282 6.01727 2.46819 6.02439 2.46572L6.21804 2.40118C6.2607 2.38698 6.29946 2.36304 6.33125 2.33125C6.36304 2.29946 6.38698 2.2607 6.40118 2.21804L6.46572 2.02439C6.46819 2.01727 6.47282 2.01109 6.47896 2.00672C6.48511 2.00235 6.49246 2 6.5 2C6.50754 2 6.51489 2.00235 6.52103 2.00672C6.52718 2.01109 6.53181 2.01727 6.53428 2.02439L6.59882 2.21804C6.61302 2.2607 6.63696 2.29946 6.66875 2.33125C6.70054 2.36304 6.7393 2.38698 6.78196 2.40118L6.97561 2.46572C6.98273 2.46819 6.98891 2.47282 6.99328 2.47897C6.99765 2.48511 7 2.49246 7 2.5C7 2.50754 6.99765 2.51489 6.99328 2.52104C6.98891 2.52718 6.98273 2.53181 6.97561 2.53428L6.78196 2.59882C6.6954 2.6276 6.6276 2.6954 6.59882 2.78196L6.53428 2.97561Z"
            fill="currentColor"
            className={`transition-all duration-200 ${
              currentTheme !== "system" ? "opacity-0 translate-y-1/2" : ""
            }`}
          />
          <path
            d="M5.06865 4.95042C5.06387 4.96485 5.05466 4.97741 5.04234 4.98631C5.03002 4.99521 5.0152 5 5 5C4.9848 5 4.96998 4.99521 4.95766 4.98631C4.94533 4.97741 4.93613 4.96485 4.93135 4.95042L4.80238 4.56319C4.74473 4.39056 4.60944 4.25527 4.43681 4.19761L4.04958 4.06865C4.03515 4.06387 4.02259 4.05466 4.01369 4.04234C4.00479 4.03002 4 4.0152 4 4C4 3.9848 4.00479 3.96998 4.01369 3.95766C4.02259 3.94534 4.03515 3.93613 4.04958 3.93135L4.43681 3.80239C4.52195 3.77402 4.59931 3.72622 4.66276 3.66276C4.72622 3.59931 4.77402 3.52195 4.80239 3.43681L4.93135 3.04958C4.93613 3.03515 4.94534 3.02259 4.95766 3.01369C4.96998 3.00479 4.9848 3 5 3C5.0152 3 5.03002 3.00479 5.04234 3.01369C5.05466 3.02259 5.06387 3.03515 5.06865 3.04958L5.19761 3.43681C5.22598 3.52195 5.27378 3.59931 5.33724 3.66276C5.40069 3.72622 5.47805 3.77402 5.56319 3.80239L5.95042 3.93135C5.96485 3.93613 5.97741 3.94534 5.98631 3.95766C5.99521 3.96998 6 3.9848 6 4C6 4.0152 5.99521 4.03002 5.98631 4.04234C5.97741 4.05466 5.96485 4.06387 5.95042 4.06865L5.56319 4.19761C5.47805 4.22598 5.40069 4.27378 5.33724 4.33724C5.27378 4.40069 5.22598 4.47805 5.19761 4.56319L5.06865 4.95042Z"
            fill="currentColor"
            className={`transition-all duration-200 ${
              currentTheme !== "system" ? "opacity-0 translate-y-1/2" : ""
            }`}
          />
        </svg>
      </label>
    </div>
  );
}

